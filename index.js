...
var leaderboard = mysqlTable("leaderboard", {
  id: int("id").autoincrement().primaryKey(),
...
var leaderboardRouter = router({
  ...
  getAll: publicProcedure.query(async () => {
    ...
    const players = await db.select().from(leaderboard).orderBy(desc(leaderboard.totalTokens));
    return players;
  }),
  // Get a single player by wallet address
  getPlayer: publicProcedure.input(z2.object({ walletAddress: z2.string() })).query(async ({ input }) => {
    const db = await getDb();
    if (!db) {
      return null;
    }
    const player = await db.select().from(leaderboard).where(eq2(leaderboard.walletAddress, input.walletAddress)).limit(1);
    return player.length > 0 ? player[0] : null;
  }),
  // Update or create player entry
  upsertPlayer: publicProcedure.input(
    z2.object({
      walletAddress: z2.string(),
      totalTokens: z2.number(),
      lastGameTaps: z2.number().optional(),
      lastGameTokens: z2.number().optional()
    })
  ).mutation(async ({ input }) => {
    const db = await getDb();
    if (!db) {
    ...
    const existingPlayer = await db.select().from(leaderboard).where(eq2(leaderboard.walletAddress, input.walletAddress)).limit(1);
    if (existingPlayer.length > 0) {
      ...
      }).where(eq2(leaderboard.walletAddress, input.walletAddress));
    } else {
...
var appRouter = router({
  ...
  // Leaderboard router for tracking player earnings
